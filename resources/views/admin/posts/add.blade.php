    ...
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            <div class="max-w-2xl lg:mx-0">
                <h2 class="text-3xl font-bold tracking-tight text-gray-900 sm:text-4xl">Add Story</h2>
                <p class="mt-2 text-lg leading-8 text-gray-600">Create My New Story Here
                </p>
            </div>
        </h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="p-8 bg-white shadow-sm sm:rounded-lg mb-8">
                <form action="{{ route('add.posts.store') }}" method="POST">
                    @csrf
                    <div class="grid grid-cols-1 gap-y-6 sm:grid-cols-2">
                        ...
                        <div class="col-span-2">
                            ...
                            <input type="text" name="tags" id="tags" class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" value="{{ old('tags') }}">
                            <small class="block text-xs text-gray-500">Separated with comma</small>
                        </div>

                       <div class="col-span-2">
                            <label for="post" class="block text-sm font-medium text-gray-700">Story</label>
                            <textarea id="post" name="post"
                            class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">{{ old('post') }}</textarea>
                            @error('post')
                            <span class="font-medium text-red-600" role="alert">{{ $message }}</span>
                            @enderror
                        </div>
